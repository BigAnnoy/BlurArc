...
服务器直接返回内联 HTML（无前端构建依赖，无外部资源引用）。暗色主题，与 Blur Arc 品牌一致，纯原生 JS 实现。

#### 4.6.1 页面布局

```
┌──────────────────────────────────┐
│          🌐 Blur Arc             │  ← 顶部品牌栏（暗色背景 + Logo 文字）
│       从手机导入照片              │
├──────────────────────────────────┤
│                                  │
│    ┌──────────────────────┐      │
...
- `<input type="file" multiple accept="image/*,video/*" capture>` — `capture` 属性允许直接调起相机拍照
